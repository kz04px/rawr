...
#[must_use]
const fn permute(set: u64, subset: u64) -> u64 {
...
const fn calculate_bishop_moves(sq: i32, blockers: u64) -> u64 {
    let mut result = 0u64;

    // Up 1 Right 1
    let mut bb = 1u64 << sq;
    while (!blockers & bb) != 0u64 {
        bb = north(east(bb));
        result |= bb;
    }
...
#[must_use]
const fn calculate_rook_moves(sq: i32, blockers: u64) -> u64 {
    let mut result = 0u64;
...
#[must_use]
fn generate_magic_moves() -> Vec<u64> {
    let mut result = vec![0u64; 88772];
    ...
    while i < 64 {
        ...
        let mut perm = 0u64;
        loop {
            let index = bishop_index(i as i32, perm);
            result[index] = calculate_bishop_moves(i as i32, perm);
        ...
        let mut perm = 0u64;
        loop {
            let index = rook_index(i as i32, perm);
            result[index] = calculate_rook_moves(i as i32, perm);
...
fn main() -> std::io::Result<()> {
    ...
    let magics = generate_magic_moves();

    // Ghetto tests -- directions
    assert_eq!(north(0x1000000), 0x100000000);
    assert_eq!(east(0x1000000), 0x2000000);
    assert_eq!(west(0x1000000), 0x0);
    assert_eq!(south(0x1000000), 0x10000);
    assert_eq!(north(0x80000000), 0x8000000000);
    assert_eq!(east(0x80000000), 0x0);
    assert_eq!(west(0x80000000), 0x40000000);
    assert_eq!(south(0x80000000), 0x800000);
    // Ghetto tests -- masks
    assert_eq!(BISHOP_MASKS[0], 0x40201008040200);
    assert_eq!(BISHOP_MASKS[28], 0x2442800284400);
    assert_eq!(ROOK_MASKS[0], 0x101010101017e);
    assert_eq!(ROOK_MASKS[28], 0x1010106e101000);
    // Ghetto tests -- bishop moves
    assert_eq!(calculate_bishop_moves(0, 0x0), 0x8040201008040200);
    assert_eq!(calculate_bishop_moves(0, 0x1000000000), 0x1008040200);
    assert_eq!(calculate_bishop_moves(28, 0x440000004400), 0x442800284400);
    // Ghetto tests -- rook moves
    assert_eq!(calculate_rook_moves(0, 0x0), 0x1010101010101fe);
    assert_eq!(calculate_rook_moves(0, 0x10008), 0x1010e);
    // Ghetto tests -- bishop indices
    ...
    assert_eq!(rook_index(2, 0x8060800000008), 38936);
    // Ghetto tests -- magic moves
    assert_eq!(magics[0], 0x808f70808080808);
    assert_eq!(magics[1], 0x808f40800000000);
    assert_eq!(magics[2], 0x0);
    assert_eq!(magics[100], 0x808340808080000);
    assert_eq!(magics[101], 0x8360808080800);
    assert_eq!(magics[102], 0x8360808080808);

    let out_dir = env::var_os("OUT_DIR").unwrap();
    let dest_path = Path::new(&out_dir).join("magic_constants.rs");
    let mut file = File::create(dest_path)?;

    write!(file, "const MAGIC_MOVES: [u64; 88772] = [\n    ")?;
    for magic in magics {
        write!(file, "{magic},")?;
    }
    write!(file, "\n];\n")?;
...
//         assert!(bishop_masks[0] == 0x8040201008040200);
//         assert!(bishop_masks[7] == 0x102040810204000);
//         assert!(bishop_masks[56] == 0x2040810204080);
//         assert!(bishop_masks[63] == 0x40201008040201);
//         assert!(bishop_masks[28] == 0x182442800284482);
//     }

//     #[test]
//     fn test_rook_masks() {
//         assert!(rook_masks[0] == 0x1010101010101fe);
//         assert!(rook_masks[7] == 0x808080808080807f);
//         assert!(rook_masks[56] == 0xfe01010101010101);
//         assert!(rook_masks[63] == 0x7f80808080808080);
//         assert!(rook_masks[28] == 0x10101010ef101010);
//     }
...
//         assert_eq!(bishop_index(0, 0x0), 5378);
//         assert_eq!(bishop_index(0, 0x0), 123);
//     }
// }
